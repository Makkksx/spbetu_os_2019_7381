...
CODE	SEGMENT
;----------------------------
OVL_SIZE PROC ;Îïðåäåëÿåì ðàçìåð îâåðëåÿ
	push es
	push bx
	push si
	push ds
	...
	mov ds, dx
	lea dx, DTA	
	mov ax, 1A00h ;Óñòàíîâêà àäðåñà äëÿ DTA
	int 21h
	pop dx
	pop ds
	...
	mov dx, seg DTA_path	
	mov ds, dx
	lea dx, DTA_path ;Óêàçàòåëü íà ñòðîêó, ñîäåðæàùóþ ïóòü ê îâåðëåþ
	mov ax, 4E00h
	int 21h
	pop dx
	pop ds
...
without_err:
	...
	push si
	lea si, DTA
	add si, 1Ch	;Â áóôåðå ñòàðøåå ñëîâî ðàçìåðà ôàéëà
	mov bx, [si]	
	sub si, 2	
	mov bx, [si]
	push cx
	mov cl, 4
	shr bx, cl ;Ïåðåâîäèì â ïàðàãðàôû 
	pop cx
	mov ax, [si+2] 
	push cx
	mov cl, 12
	sal ax, cl	;Ïåðåâîäèì â áàéòû, à çàòåì â ïàðàãðàôû
	pop cx
	add bx, ax	
	inc bx
	inc bx
		
	mov ax, 4800h ;Âûäåëåíèå ïàìÿòè
	int 21h			
	mov ovl_adr, ax
	pop si
	pop bx
...
CALL_OVL  	ENDP
;----------------------------
OUTPUT_PROC PROC NEAR ;Âûâîä íà ýêðàí ñîîáùåíèÿ
	push ax
	mov  ah, 09h
	int  21h
	pop	 ax
